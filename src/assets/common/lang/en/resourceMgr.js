...
export default {
    ...
    clusterUrlContent: 'Enter the IP of all CM/CM_VDI nodes in http or https format', // 输入所有CM/CM_VDI节点的IP，格式http或https
    ...
    clusterAdmin: 'cluster administrator', // 用户名
    clusterAdminPassword: 'password', // 用户名
    ...
    vdcallocationCpuEditMsg: 'The number of allocated CPUs cannot be greater than the number of available upper levels, and cannot be less than the number of allocated lower levels', // '分配CPU数不能大于上级可用数，并且不能小于已使用数'
    vdcallocationCpuAddMsg: 'The number of allocated CPUs is a positive integer greater than 1 and less than the number of available CPUs', // '分配CPU数不能大于上级可用数，并且不能小于已使用数'
    vdcallocationCpuMinMsg: 'The number of allocated CPUs cannot be less than the number of used CPUs', // '分配CPU数不能小于已使用数'
    vdcallocationMemEditMsg: 'The number of allocated memory cannot be greater than the number of available upper levels, and cannot be less than the number of allocated lower levels', // '分配内存不能大于上级可用数，并且不能小于已使用数'
    vdcallocationMemAddMsg: 'The number of allocated memory is a positive integer greater than 1 and less than the number of available memory', // '分配CPU数不能大于上级可用数，并且不能小于已使用数'
    vdcallocationMemMinMsg: 'The number of allocated memory cannot be less than the number of used memory', // '分配内存数不能小于已使用数'
    allocationStorageAddMsg: 'The allocated storage size is a positive integer greater than 1 and less than the amount of available storage', // 分配存储大小为大于1的正整数，且小于可用存储数
    allocationStorageEditMsg: 'The number of allocated storage cannot be greater than the number of available upper levels, and cannot be less than the number of allocated lower levels', // 分配存储数不能大于上级可用数，并且不能小于已使用数
    allocationStorageMinMsg: 'The number of allocated storage cannot be less than the number of used', // '分配存储数不能小于已使用数'
    ...
    vdcallocatedtooltip: 'Allocate resource quotas, which can exceed the total amount of resources', // 分配资源额度,其可超过资源总量
    vdcSupCanassigned: 'Can be assigned by superiors', // 上级可分配
    firstvdcSupCanassigned: 'total resources', // 资源总量
    vdcAllocateCPUused: 'The number of allocated CPUs should be greater than the number of used CPUs', // 分配CPU数应大于已使用CPU数
    vdcAllocateCPUequal: 'The number of allocated CPUs is a positive integer greater than or equal to 1', // 分配CPU数为大于等于1的正整数
    vdcAllocateMemused: 'The amount of allocated memory should be greater than the amount of used memory', // 分配内存数应大于已使用内存数
    vdcAllocateMemequal: 'The number of allocated memory is a positive integer greater than or equal to 1', // 分配内存数为大于等于1的正整数
    vdcAllocateStorageused: 'Allocated storage should be greater than used storage', // 分配存储数应大于已使用存储数
    vdcAllocateStorageequal: 'Allocate storage size as a positive integer greater than or equal to 1', // 分配存储大小为大于等于1的正整数
    vdcauditOpinion: 'Reason for change', // 变更原因不能为空
    vdcauditOpinionEmpt: 'Change reason cannot be empty', // 变更原因不能为空
    vdcauditOpinionspecialChar: 'The reason for change can only be a combination of Chinese, English and numbers', // 变更原因只能由中文、英文、数字组合
    ...
    deleteconfirmMsg: 'Are you sure you want to delete the selected cloud server?',// 确定要删除所选云服务器？
    deleteconfirmTit: 'delete',// 删除
    abortconfirmMsg: 'This operation may result in the loss of unsaved data in the cloud server. Do you want to confirm the forced shutdown?',// 此操作可能导致云服务器内尚未保存的数据丢失，是否确认强制关机？
    abortconfirmTit: 'Force shutdown',// 强制关机
    abortSucc: 'Forced shutdown succeeded!',// 强制关机成功!
    abortErr: 'Forced shutdown failed!',// 强制关机失败!

    forcedRestartconfirmMsg: 'This operation may result in the loss of unsaved data in the cloud server. Are you sure you want to force a restart?',// 此操作可能导致云服务器内尚未保存的数据丢失，是否确认强制关机？
