      ...
      <template v-for="(item, index) in columnArr">
        ...
        >
          <template slot-scope="{ row }">
            <el-link
              v-if="
                row.hasOperatePermission &&
                otherBtnShow('share_image', 'share_image_edit')
              "
              type="primary"
              @click="editInfo(row)"
            >
          ...
          <template slot-scope="{ row }">
            ...
            >
              <router-link
                :to="{
                  name: 'imageDetail',
                  params: { id: row.imageId, type: 'sharedImage' },
                }"
              >
  ...
  data() {
    return {
      ...
      toggleList: [
        {
          key: "topublicMirror",
          label: this.$t("imageMgr.topublicMirror"), //"转为公共镜像",
          disabled: false,
          show: true,
        },
        {
  ...
  methods: {
    ...
    totableData(list) {
      ...
      list.forEach((element, index) => {
        let { imageFrom, type, status, hasOperatePermission } = element;
        // 来源映射
        ...
        toggleList.forEach((toggleListelement, toggleListi) => {
          ...
          switch (key) {
            case "topublicMirror":
              ...
              toggleList[toggleListi].show =
                hasOperatePermission &&
                this.otherBtnShow("share_image", "share_image_to_public");
            ...
            case "syncImage":
              ...
              toggleList[toggleListi].show =
                hasOperatePermission &&
                this.otherBtnShow("share_image", "share_image_sync");
              break;
            case "deleteImage":
              ...
              toggleList[toggleListi].show =
                hasOperatePermission &&
                this.otherBtnShow("share_image", "share_Image_delete");
    ...
    editInfo(row) {
      this.$showFullScreenLoading(".sharedMirror");
    ...
    settopublicMirror(row) {
      ...
      imageDetail(params)
        .then((res) => {
          this.topublicmodalOption.formData = res;
